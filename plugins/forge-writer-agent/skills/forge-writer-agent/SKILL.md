...
This is a backend-only agent plugin: it preserves the writer kernel, memory, storage, provider budget, Project Brain management, metacognitive recovery, chapter-generation pipeline, chapter-state repair, and headless model-backed agent loop. The MCP entrypoint runs `cmd /c scripts\forge-agent-mcp.cmd stdio` from the repository root, which launches the compiled `forge-agent-mcp.exe` over newline-delimited JSON-RPC stdio and keeps logs on stderr. It does not provide the original Tauri/React desktop UI or renderer event stream, and editor-only renderer hooks are not plugin tools.
